...
const ROOT_SELECTOR = '[data-commerce-slider]';
const CARD_SELECTOR = '.sectionCommerceSlider01-item';
const RESULTS_UPDATED_EVENT = 'liquidstack:commerce-results-updated';
const ITEMS_APPENDED_EVENT = 'liquidstack:commerce-collection-appended';
const ROOT_OWNER = Symbol.for(
  'liquidstack.commerce.sectionCommerceSlider01.owner',
);
...
const installSlider = (root) => {
  const viewport = root.querySelector?.('[data-commerce-slider-viewport]');
  const track = root.querySelector?.('[data-commerce-slider-track]')
    ?? root.querySelector?.('.sectionCommerceSlider01-track');
  const previous = root.querySelector?.('[data-commerce-slider-previous]');
  const next = root.querySelector?.('[data-commerce-slider-next]');
  const controls = root.querySelector?.('[data-commerce-slider-controls]')
    ?? previous?.parentElement
    ?? next?.parentElement;
  const autoplayToggle = root.querySelector?.(
    '[data-commerce-slider-autoplay-toggle]',
  );
  ...
  const autoplayConfigured = root.dataset.commerceSliderAutoplay !== 'false';
  const autoplayDelay = finiteNumber(
    root.dataset.commerceSliderAutoplayDelay,
    6,
    2,
    60,
  );
  const transitionDuration = finiteNumber(
    root.dataset.commerceSliderDuration,
    1,
    0.1,
    5,
  );
  ...
  const cardElements = () => {
    const candidates = Array.from(track.children ?? []).filter(
      (element) => !element.hasAttribute?.('data-commerce-slider01-clone'),
    );
  ...
  const clearEnhancedLayout = () => {
    ...
    root.classList.remove(
      'sectionCommerceSlider01--enhanced',
      'sectionCommerceSlider01--dragging',
    );
    enhanced = false;
    loopEnabled = false;
  ...
  const cloneForLoop = (card) => {
    ...
    clone.setAttribute('data-commerce-slider01-clone', '');
    ...
    for (const element of cloneTree) {
      ...
      element.removeAttribute?.('data-commerce-card-key');
    }
    for (const element of cloneTree) {
  ...
  const finishDrag = () => {
    root.classList.remove('sectionCommerceSlider01--dragging');
    dragging = false;
    if (dragDistance > 8) {
  ...
  const installDraggable = () => {
    const options = {
      ...
      onDragStart() {
        dragging = true;
        root.classList.add('sectionCommerceSlider01--dragging');
      },
      onDrag() {
        dragging = true;
      ...
      onRelease() {
        ...
        const releasedDraggable = this;
        const settleWithoutThrow = () => {
          if (!releasedDraggable.isThrowing && dragging) {
            finishDrag();
          }
        };
        if (typeof view.requestAnimationFrame === 'function') {
          view.requestAnimationFrame(settleWithoutThrow);
        } else {
          view.setTimeout(settleWithoutThrow, 0);
        }
      },
  ...
  const rebuild = () => {
    ...
    const activeKey = cards[currentIndex]?.dataset?.commerceCardKey ?? '';
    clearEnhancedLayout();
    cards = cardElements();
    if (activeKey !== '') {
      const preservedIndex = cards.findIndex(
        (card) => card.dataset.commerceCardKey === activeKey,
      );
      if (preservedIndex >= 0) {
    ...
    root.classList.add('sectionCommerceSlider01--enhanced');
    enhanced = true;
  ...
  track.addEventListener('load', (event) => {
    if (event.target?.closest?.('[data-commerce-slider01-clone]')) {
      return;
    }
    scheduleRebuild();
  }, {
...
export const cleanupSectionCommerceSlider01 = () => {
  disposeActiveSliders();
  disposeActiveSliders = () => {};
};

export const initSectionCommerceSlider01 = (scope = globalThis.document) => {
  cleanupSectionCommerceSlider01();
...
export default initSectionCommerceSlider01;

if (import.meta.hot) {
  import.meta.hot.dispose(cleanupSectionCommerceSlider01);
}
