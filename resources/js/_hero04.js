...
export default function initHero04() {
    const roots = Array.from(document.querySelectorAll('[data-hero04]'));
    if (roots.length === 0) {
        return;
    }

    roots.forEach((root) => {
        const canvas = root.querySelector('[data-hero04-canvas]');
        if (!canvas) {
            return;
        }

        initHero04Fluid(root, canvas);
    });
}

function initHero04Fluid(root, canvasEl) {
    'use strict';
...
let config = {
    ...
    const qualitySetting = parseInt(root.dataset.hero04Quality || '1', 10);
    const qualityMap = [1024, 512, 256, 128];
    if (!Number.isNaN(qualitySetting) && qualityMap[qualitySetting] !== undefined) {
        config.DYE_RESOLUTION = qualityMap[qualitySetting];
        config.SIM_RESOLUTION = qualitySetting <= 1 ? 128 : 64;
        config.BLOOM_RESOLUTION = qualitySetting <= 1 ? 256 : 128;
        config.SUNRAYS_RESOLUTION = qualitySetting <= 1 ? 196 : 128;
    }

    config.COLORFUL = root.dataset.hero04Colorful !== 'false';
    const randomSplatsSetting = root.dataset.hero04Random === 'true';

    const bgColor = getComputedStyle(root).backgroundColor;
    const bgMatch = bgColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
    if (bgMatch) {
        config.BACK_COLOR = {
            r: parseInt(bgMatch[1], 10),
            g: parseInt(bgMatch[2], 10),
            b: parseInt(bgMatch[3], 10),
        };
    }

    const overlay = root.querySelector('.hero04-content');
    ...
    if (overlay && !prefersReduced) {
        ...
        gsap.set(overlay.children, { autoAlpha: 1, y: 0 });
    }
...
let _bgImagePath = "";
_randomSplats = randomSplatsSetting;
if (_randomSplats) {
    timer = setInterval(randomSplat, 3500);
}
...
interactionTarget.addEventListener('touchstart', e => {
    ...
    for (let i = 0; i < touches.length; i++) {
        const pos = getPointerPosition(touches[i].clientX, touches[i].clientY);
