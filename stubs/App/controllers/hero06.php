<?php
/**
 * Directrices de copy para hero06:
 * - Bloque {hero06-content}: módulo H1 o contenido equivalente inyectado.
 * - Alt/title de imagen: 5-10 palabras descriptivas.
 * El recurso controla únicamente escenario, overlay e imagen editable.
 */
function controller_hero06(int $i = 0, array $params = []): string
{
    $pad = sprintf('%02d', $i);
    ...
    $editorAttributes = $devMode
        ? 'data-inline-background'
            . ' data-inline-background-target=".hero06-media"'
    ...
    $imageHeight = max(1, (int) (
        is_object($imageObj) && isset($imageObj->height) ? $imageObj->height : 1600
    ));

    $vars = [
        '{classVar}'          => "hero06_{$pad}_classVar",
